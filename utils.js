    ...
    this.showFull = function() {
        for (i = 0; i < objectOne.lPoints.length; i++) {
            ...
            if (objectOne.lPoints[i].tp == "SAUDE") {
                ...
                objectOne.lPoints[i].contentString = '<div id="content" style="background:#148CE2;"><h1>' + localPoint.title + '</h1><br><img style="border-radius: 15%;width:120px" src=' + localPoint.pic + '/>' + 'Preço' + localPoint.preco + '<br><a href=property-details.php?id=' + localPoint.id + '>Ver mais</a></div>';
            ...
            objectOne.lPoints[i].marker.addListener('click', function() {
                infowindow.open(map, objectOne.lPoints[i].marker);
            });
